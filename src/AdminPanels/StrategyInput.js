...
export default function StrategyInput({ player, changeRender, strategies }) {
    const [state, setState] = useState({ numberOfStrategies: [1, 2] });
    const handleNextClick = (e, number) => {
        e.preventDefault();

        if (state.player === 1) {
            if (localStorage.getItem("firstStrategies") === null)
                setState({ numberOfStrategies: [1, 2] });
        }
    ...
    return (
        <div className="StrategyInput" id="firstplayerstrategies">
            ...
            <div className="pageMover">
                ...
                <div className="next">
                    <p>{player === 1 ? "Pređi na sledećeg igrača" : "Sačuvaj igru i pređi na unos isplata"}</p>
